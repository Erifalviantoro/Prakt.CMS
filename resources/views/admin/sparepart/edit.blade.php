...
<main class="ml-64 min-h-screen flex flex-col">
    ...
    <!-- Header: Menyelaraskan Struktur Navigasi Atas -->
    <header class="sticky top-0 z-50 flex items-center justify-between px-6 py-3 w-full bg-surface-container-lowest dark:bg-surface-container-low shadow-sm dark:shadow-none border-b border-outline-variant">
        <div class="flex items-center gap-4">
            <h2 class="font-headline-md text-headline-md font-bold text-primary dark:text-primary-fixed">Edit Sparepart</h2>
    ...
    <section class="p-8 flex-1">
        <div class="max-w-container-max-width mx-auto">
            ...
            <nav class="flex items-center gap-2 mb-8 text-on-surface-variant text-label-md">
                ...
                <span class="material-symbols-outlined text-[16px]">chevron_right</span>
                <span class="text-primary font-bold">Edit: {{ $sparepart->nama }}</span>
            </nav>
            ...
            <!-- Form Mengarah ke Update Route -->
            <form action="{{ route('admin.sparepart.update', $sparepart->id) }}" class="bento-grid grid grid-cols-12 gap-6" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input class="w-full px-4 py-3 rounded-lg border border-outline-variant bg-surface-container-low text-body-md focus:ring-2 focus:ring-primary/20 transition-all @error('nama') border-red-500 @enderror" id="nama" name="nama" placeholder="Contoh: Kampas Rem Depan Vario 125" required type="text" value="{{ old('nama', $sparepart->nama) }}"/>
                                    @error('nama') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                
                                <!-- Input Supplier (Bawaan database otomatis terpilih) -->
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <select class="w-full px-4 py-3 rounded-lg border border-outline-variant bg-surface-container-low text-body-md appearance-none focus:ring-2 focus:ring-primary/20 transition-all" id="supplier" name="supplier">
                                            ...
                                            <option value="1" {{ old('supplier', $sparepart->supplier ?? '') == '1' ? 'selected' : '' }}>AHM Genuine Parts</option>
                                            <option value="2" {{ old('supplier', $sparepart->supplier ?? '') == '2' ? 'selected' : '' }}>Indoparts Distribution</option>
                                            <option value="3" {{ old('supplier', $sparepart->supplier ?? '') == '3' ? 'selected' : '' }}>Federal Parts Center</option>
                                            <option value="4" {{ old('supplier', $sparepart->supplier ?? '') == '4' ? 'selected' : '' }}>PT Mitra Kencana</option>
                                        </select>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        <input class="w-full px-4 py-3 rounded-lg border border-outline-variant bg-surface-container-low text-body-md focus:ring-2 focus:ring-primary/20 transition-all @error('stok') border-red-500 @enderror" id="stok" min="0" name="stok" placeholder="0" required type="number" value="{{ old('stok', $sparepart->stok) }}"/>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input class="w-full pl-12 pr-4 py-3 rounded-lg border border-outline-variant bg-surface-container-low text-body-md focus:ring-2 focus:ring-primary/20 transition-all @error('harga') border-red-500 @enderror" id="harga" name="harga" placeholder="0" required type="number" min="0" value="{{ old('harga', intval($sparepart->harga)) }}"/>
                                    </div>
                ...
                <div class="col-span-12 lg:col-span-4 space-y-6">
                    ...
                    <div class="bg-surface-container-lowest p-8 rounded-xl border border-outline-variant shadow-sm transition-all hover:shadow-md">
                        ...
                        <div class="relative group cursor-pointer border-2 border-dashed border-outline-variant rounded-xl p-4 transition-all hover:border-primary hover:bg-primary/5">
                            ...
                            <!-- Placeholder Upload: Disembunyikan otomatis oleh JS jika data foto bawaan ada -->
                            <div class="{{ $sparepart->foto ? 'hidden' : '' }} aspect-square bg-surface-container-low rounded-lg overflow-hidden flex flex-col items-center justify-center gap-4 text-center p-6" id="upload-placeholder">
                                <span class="material-symbols-outlined text-[48px] text-outline group-hover:text-primary transition-colors">cloud_upload</span>
                                <div>
                                    <p class="text-label-md font-bold text-primary">Klik untuk ganti foto</p>
                                    <p class="text-label-sm text-on-surface-variant mt-1">PNG, JPG, atau WebP (Maks. 2MB)</p>
                                </div>
                            </div>

                            <!-- Preview Container: Menyesuaikan status gambar tersimpan asli di storage database -->
                            <div class="{{ $sparepart->foto ? '' : 'hidden' }} absolute inset-4 bg-surface-container-low rounded-lg overflow-hidden" id="preview-container">
                                <img alt="Preview" class="w-full h-full object-cover" id="image-preview" src="{{ $sparepart->foto ? asset('storage/' . $sparepart->foto) : '#' }}"/>
                    ...
                    <!-- Panel Konfirmasi & Update -->
                    <div class="bg-surface-container-lowest p-8 rounded-xl border border-outline-variant shadow-sm">
                        <p class="text-label-sm text-on-surface-variant mb-6 italic">Pastikan semua data sudah benar sebelum memperbarui informasi ke dalam sistem inventaris.</p>
                    ...
                    <div class="p-6 bg-primary-fixed text-on-primary-fixed rounded-xl flex gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-label-sm mt-1">Mengubah harga jual akan langsung memperbarui kalkulasi otomatis pada kwitansi mekanik dan riwayat POS kasir.</p>
                        </div>
                    </div>
                </div>
            </form>
